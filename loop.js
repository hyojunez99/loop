...
const handleNextClick = () => {
  ...
  disableButtons();
  count++;
  // sliderElem의 위치값 변화 : left:-100%
  sliderElem.style.transition = "0.5s";
  sliderElem.style.left = `-${count * 100}%`;
};
// prev 버튼을 클릭했을 때
prevBtn.addEventListener("click", hadnlePrevClick);
// next 버튼을 클릭했을 때
nextBtn.addEventListener("click", handleNextClick);
// css에서 트랜지션이 끝났을 때 발생되는 이벤트
// transitionend
sliderElem.addEventListener("transitionend", () => {
  // 순간 이동처리 마지막 MAX_SIZE-1가 되면 첫번째로
  if (count === MAX_SIZE - 1) {
    sliderElem.style.transition = "none";
    count = 1;
    sliderElem.style.left = `-${count * 100}%`;
  }
  // 순간 이동처리 맨 앞이 0 번이 보여지면 이동
  if (count === 0) {
    sliderElem.style.transition = "none";
    count = MAX_SIZE - 2;
    sliderElem.style.left = `-${count * 100}%`;
